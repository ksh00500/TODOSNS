...
type DirectChatDraft = { body?: string; files: File[]; replyToId?: string; version: number };
...
export default function DirectChatPage() {
  ...
  const read = useMutation({ mutationFn: (messageId: string) => apiFetch(`/messages/${id}/read`, { method: "POST", body: JSON.stringify({ messageId }) }), onSuccess: () => { void client.invalidateQueries({ queryKey: ["chat-inbox"] }); void client.invalidateQueries({ queryKey: ["direct-conversations"] }); void client.invalidateQueries({ queryKey: ["direct-unread-count"] }); } });
  const markRead = read.mutate;
  const send = useMutation({
    mutationFn: async (draft: DirectChatDraft) => { const mediaIds: string[] = []; setProgress(draft.files.map(() => 0)); for (const [index, file] of draft.files.entries()) mediaIds.push(await uploadImage(file, (value) => setProgress((current) => current.map((item, itemIndex) => itemIndex === index ? value : item)))); return apiFetch(`/messages/${id}/messages`, { method: "POST", body: JSON.stringify({ body: draft.body, mediaIds, replyToId: draft.replyToId }) }); },
  ...
  const react = useMutation({ mutationFn: ({ messageId, type }: { messageId: string; type: ChatReactionType }) => apiFetch(`/messages/${id}/messages/${messageId}/reactions`, { method: "POST", body: JSON.stringify({ type }) }), onSuccess: async () => { setReactionFor(null); await refresh(); }, onError: (cause) => setNotice(userErrorMessage(cause, "반응을 저장하지 못했어요.")) });
  ...
  function submit(event: FormEvent) { event.preventDefault(); send.mutate({ body: body.trim() || undefined, files: [...files], replyToId: replying?.id, version: draftVersionRef.current }); }
  function chooseFiles(list: FileList | null) { const selected = Array.from(list ?? []); if (selected.length > 4) return setNotice("사진은 메시지마다 최대 4장까지 올릴 수 있어요."); if (selected.some((file) => file.size > 10_000_000)) return setNotice("사진은 한 장당 10MB 이하여야 해요."); draftVersionRef.current += 1; setFiles(selected.slice(0, 4)); setProgress([]); }
  ...
  return <main className="chat-page direct-chat-page">
    ...
    <section className="chat-messages">{!items.length ? <EmptyState title="첫 메시지를 보내보세요" body="가볍게 인사하며 대화를 시작해보세요." /> : items.map((message) => <DirectBubble key={message.id} message={message} revealed={revealedBlocked.has(message.id)} onReveal={() => setRevealedBlocked((current) => new Set(current).add(message.id))} onReply={() => { draftVersionRef.current += 1; setReplying(message); }} onMenu={() => setMenuFor(message.id)} onReact={() => setReactionFor(message.id)} onMedia={setViewingMedia} />)}</section>
    ...
    {!room.canSend ? <div className="chat-readonly"><Ban /><span><b>메시지를 보낼 수 없는 대화예요</b><small>{room.blockedByMe ? "내가 차단한 사용자예요." : "상대방의 개인 정보 설정으로 대화를 이어갈 수 없어요."}</small></span></div> : <form className="chat-composer" onSubmit={submit}>
      {replying && <div className="chat-compose-context"><span><MessageCircleReply /></span><div><b>{replying.sender?.nickname ?? "메시지"}에게 답장</b><small>{replying.body || "사진 메시지"}</small></div><button type="button" aria-label="답장 취소" onClick={() => { draftVersionRef.current += 1; setReplying(null); }}><X /></button></div>}
      {files.length > 0 && <div className="chat-photo-drafts">{files.map((file, index) => <PhotoDraft file={file} index={index} progress={progress[index]} key={`${file.name}-${file.lastModified}-${index}`} onRemove={() => { draftVersionRef.current += 1; setFiles((current) => current.filter((_, itemIndex) => itemIndex !== index)); }} />)}</div>}
      <div className="chat-compose-row"><label className="chat-photo-button" aria-label="사진 첨부"><input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={(event) => chooseFiles(event.target.files)} /><ImagePlus /></label><label className="sr-only" htmlFor="direct-chat-message">대화 메시지</label><textarea id="direct-chat-message" rows={1} value={body} onChange={(event) => { draftVersionRef.current += 1; setBody(event.target.value); }} maxLength={2000} placeholder="메시지를 입력하세요" /><button className="chat-send-button" aria-label="메시지 보내기" disabled={send.isPending || (!body.trim() && !files.length)}>{send.isPending ? <LoaderCircle className="spin" /> : <Send />}</button></div>
    </form>}
    ...
    {menuFor && <DirectMessageMenu message={items.find((item) => item.id === menuFor)!} onClose={() => setMenuFor(null)} onReply={() => { draftVersionRef.current += 1; setReplying(items.find((item) => item.id === menuFor)!); setMenuFor(null); }} onDelete={() => { setDeleting(menuFor); setMenuFor(null); }} onReport={() => { setReporting(menuFor); setMenuFor(null); }} />}
    ...
    {blocking && <ConfirmSheet title={`${room.otherUser.nickname}님을 차단할까요?`} body="서로 팔로우가 해제되고 더 이상 메시지를 주고받을 수 없어요." confirmLabel="차단하기" danger busy={block.isPending} error={block.isError ? userErrorMessage(block.error, "차단하지 못했어요.") : ""} onClose={() => setBlocking(false)} onConfirm={() => block.mutate()} />}
