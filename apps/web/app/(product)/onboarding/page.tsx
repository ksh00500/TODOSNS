...
export default function OnboardingPage() {
  const { status, refresh } = useSession(); const router = useRouter(); const [selected, setSelected] = useState<string[]>([]); const mutation = useMutation({ mutationFn: () => apiFetch("/me", { method: "PATCH", body: JSON.stringify({ interests: selected, onboardingCompleted: true }) }), onSuccess: async () => { await refresh(); router.replace("/today"); } });
