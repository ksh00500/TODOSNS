...
export default function TodayPage() {
  ...
  const complete = useMutation({ mutationFn: (todo: TodoDto) => apiFetch(`/todos/${todo.id}/complete`, { method: "POST", body: JSON.stringify({ share: false }) }), onMutate: async (todo) => { await queryClient.cancelQueries({ queryKey: ["todos", date] }); const previous = queryClient.getQueryData<TodoDto[]>(["todos", date]); queryClient.setQueryData<TodoDto[]>(["todos", date], (items = []) => items.map((item) => item.id === todo.id ? { ...item, completedAt: new Date().toISOString() } : item)); return { previous }; }, onError: (_error, _todo, context) => { queryClient.setQueryData(["todos", date], context?.previous); setNotice("완료를 저장하지 못했어요."); }, onSuccess: (_result, todo) => { setShareTodo({ ...todo, completedAt: new Date().toISOString() }); setNotice("해냈어요! 이 실천을 공유할 수 있어요."); }, onSettled: () => void queryClient.invalidateQueries({ queryKey: ["todos"] }) });
  const uncomplete = useMutation({ mutationFn: (todo: TodoDto) => apiFetch(`/todos/${todo.id}/uncomplete`, { method: "POST" }), onMutate: async (todo) => { await queryClient.cancelQueries({ queryKey: ["todos", date] }); const previous = queryClient.getQueryData<TodoDto[]>(["todos", date]); queryClient.setQueryData<TodoDto[]>(["todos", date], (items = []) => items.map((item) => item.id === todo.id ? { ...item, completedAt: null } : item)); return { previous }; }, onError: (_error, _todo, context) => { queryClient.setQueryData(["todos", date], context?.previous); setNotice("완료 취소를 저장하지 못했어요."); }, onSuccess: () => { setShareTodo(null); setNotice("완료를 취소했어요."); }, onSettled: () => void queryClient.invalidateQueries({ queryKey: ["todos"] }) });
  const share = useMutation({ mutationFn: (data: { caption: string; visibility: string; mediaId?: string }) => apiFetch(`/todos/${shareTodo?.id}/complete`, { method: "POST", body: JSON.stringify({ share: true, ...data }) }), onSuccess: () => { setShareTodo(null); setNotice("오늘의 실천을 공유했어요."); void queryClient.invalidateQueries({ queryKey: ["feed"] }); } });
