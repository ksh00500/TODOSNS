...
function ExploreSearchContent() {
  ...
  const resolvedTab: SearchTab = requestedTab && SEARCH_TABS.includes(requestedTab) ? requestedTab : "all";
  const searchTab: SearchTab = searchQuery.startsWith("#") && resolvedTab === "tags" ? "all" : resolvedTab;
  ...
  const cheer = useMutation({ mutationFn: (post: FeedPost) => apiFetch(`/feed/posts/${post.id}/cheer`, { method: "POST" }), onMutate: (post) => { setActionError(""); if (status !== "authenticated") { requireLogin(); throw new Error("LOGIN_REQUIRED"); } return post; }, onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["public-search", searchQuery] }); }, onError: (error) => { if (error instanceof Error && error.message === "LOGIN_REQUIRED") return; setActionError(userErrorMessage(error, "응원을 저장하지 못했어요.")); } });
  ...
  const runSearch = (value: string, preferredTab?: SearchTab) => {
    ...
    const inferred = query.startsWith("#") ? preferredTab ?? "all" : query.startsWith("@") ? "users" : preferredTab ?? "all";
  ...
  return <main className="app-page explore-search-page"><header className="search-page-header"><Link href="/explore" aria-label="탐색으로 돌아가기"><ArrowLeft aria-hidden /><span>탐색</span></Link><div><span>실천 찾기</span><h1>검색</h1></div></header><SearchBox key={searchQuery} initialValue={searchQuery} onSearch={runSearch} onClear={() => replaceParams({ query: null, type: null })} />{actionError && <p className="inline-action-error" role="alert">{actionError}</p>}
    {searchQuery ? <SearchExperience query={searchQuery} tab={searchTab} data={search.data} loading={search.isLoading} error={search.isError} onRetry={() => void search.refetch()} onTab={(tab) => replaceParams({ type: tab })} onSearch={runSearch} onPerson={(handle) => router.push(`/people/${handle}`)} onChallenge={(id) => router.push(`/challenges/${id}`)} onCheer={(post) => cheer.mutate(post)} onCopy={openImport} pending={cheer.isPending} /> : <section className="search-discovery" aria-label="검색 추천">
      ...
      {suggestions.isLoading ? <div className="search-tab-skeleton" /> : suggestions.data?.trendingTags.length ? <div className="search-discovery-block"><div><Sparkles aria-hidden /><b>요즘 많이 찾는 태그</b></div><div className="trending-tag-grid">{suggestions.data.trendingTags.map((tag, index) => <button key={tag.id} onClick={() => runSearch(`#${tag.name}`, "all")}><i>{index + 1}</i><span>#{tag.name}<small>{tag.postCount}개 게시물</small></span><ArrowRight aria-hidden /></button>)}</div></div> : suggestions.isError ? <ErrorState onRetry={() => void suggestions.refetch()} /> : <EmptyState title="아직 인기 태그가 없어요" body="새로운 실천이 게시되면 여기에서 소개할게요." />}
    </section>}
  </main>;
}
...
function SearchExperience({ query, tab, data, loading, error, pending, onRetry, onTab, onSearch, onPerson, onChallenge, onCheer, onCopy }: { query: string; tab: SearchTab; data?: SearchResults; loading: boolean; error: boolean; pending: boolean; onRetry: () => void; onTab: (tab: SearchTab) => void; onSearch: (value: string, tab?: SearchTab) => void; onPerson: (handle: string) => void; onChallenge: (id: string) => void; onCheer: (post: FeedPost) => void; onCopy: (post: FeedPost) => void }) {
  ...
  return <section className="search-results" aria-label={`${query} 검색 결과`}><div className="search-summary"><span><Search aria-hidden /></span><div><b>‘{query}’ 검색 결과</b><small>{data.counts.all}개의 공개 결과를 찾았어요</small></div></div><div className="search-tabs" role="group" aria-label="검색 결과 종류">{tabs.map((item) => <button key={item.id} aria-pressed={tab === item.id} className={tab === item.id ? "active" : ""} onClick={() => onTab(item.id)}>{item.label}<span>{item.count}</span></button>)}</div>
    {data.counts.all === 0 ? <EmptyState title="검색 결과가 없어요" body="다른 단어를 쓰거나 #해시태그로 찾아보세요." /> : activeCount === 0 ? <EmptyState title="이 종류의 결과는 없어요" body="통합 결과에서 다른 종류를 확인해보세요." /> : <div className="search-result-content">
      ...
      {(tab === "all" || tab === "tags") && data.tags.length > 0 && <ResultSection title="해시태그" icon={Hash} count={data.counts.tags} onMore={tab === "all" ? () => onTab("tags") : undefined}><div className="hashtag-results">{data.tags.map((tag) => <button key={tag.id} onClick={() => onSearch(`#${tag.name}`, "all")}><Hash aria-hidden /><span><b>#{tag.name}</b><small>{tag.postCount}개 공개 게시물</small></span></button>)}</div></ResultSection>}
