...
export function ShareSheet({ todo, list, busy, onClose, onShare }: { todo?: TodoDto | null; list?: TodoListDto | null; busy?: boolean; onClose: () => void; onShare: (data: { caption: string; visibility: Visibility; mediaKey?: string }) => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(""); let mediaKey: string | undefined; try { if (file) { setUploading(true); mediaKey = await uploadImage(file, setProgress); } onShare({ caption, visibility, mediaKey }); } catch (cause) { setError(cause instanceof Error ? cause.message : "사진을 올리지 못했어요."); } finally { setUploading(false); } };
