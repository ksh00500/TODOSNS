...
export function FeedCard({ post, onCheer, onCopy, pending }: { post: FeedPost; onCheer: () => void; onCopy: () => void; pending?: boolean }) {
  ...
  return <article className="feed-card">
    ...
    <div className="feed-body"><div className="feed-todo-title"><span>{category}</span><Link href={`/posts/${post.id}`}>{todoTitle}</Link><small>{post.todoList ? `TODO ${post.todoList.items.length}개` : "단일 TODO"}</small></div>{post.caption && <p>{post.caption}</p>}<div className="feed-copy-count"><Check /><span>{post.copyCount}명이 자신의 하루에 담았어요</span></div></div>
