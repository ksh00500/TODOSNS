...
export class MediaService {
  ...
  async presign(userId: string, dto: PresignDto) {
    ...
    const key = `uploads/${userId}/${new Date().toISOString().slice(0, 10)}/${randomUUID()}${extensions[dto.mimeType]}`;
  ...
  viewUrl(key: string) { return getSignedUrl(this.s3, new GetObjectCommand({ Bucket: this.bucket, Key: key }), { expiresIn: 900 }); }
}
